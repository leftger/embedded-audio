...
fn run() -> Result<(), String> {
    ...
    let mut gen_rust: Option<PathBuf> = None;
    let mut gen_c: Option<PathBuf> = None;
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--gen-rust" => {
                i += 1;
                gen_rust = Some(PathBuf::from(args.get(i).ok_or("--gen-rust needs PATH")?));
            }
            "--gen-c" => {
                i += 1;
                gen_c = Some(PathBuf::from(args.get(i).ok_or("--gen-c needs PATH")?));
            }
            "--normalize" => {
                normalize = true;
            }
    ...
    for spec in &adds {
        apply_add(&mut builder, spec, normalize)?;
    }
    ...
    if let Some(path) = gen_rust {
        let mut content = String::from("// Auto-generated by eaf-bake\n\npub mod sfx {\n");
        for spec in &adds {
        ...
        println!("Generated Rust constants → {}", path.display());
    }

    if let Some(path) = gen_c {
        let mut content = String::from(
            "/* Auto-generated by eaf-bake */\n#ifndef SFX_BANK_H\n#define SFX_BANK_H\n\n",
        );
        for spec in &adds {
            content.push_str(&format!("#define SFX_{} {}\n", spec.id, spec.id));
        }
        content.push_str("\n#endif /* SFX_BANK_H */\n");
        fs::write(&path, content).map_err(|e| e.to_string())?;
        println!("Generated C header constants → {}", path.display());
    }
...
fn normalize_samples(bytes: &mut [u8]) {
    let max_dev = bytes
        .iter()
        .map(|&b| (b as i16 - 128).abs())
        .max()
        .unwrap_or(0);
    if max_dev > 0 && max_dev < 127 {
        for b in bytes.iter_mut() {
            let dev = *b as i16 - 128;
            let norm = (dev * 127) / max_dev;
            *b = (128 + norm).clamp(0, 255) as u8;
        }
    }
}

fn apply_add(builder: &mut BankBuilder, spec: &AddSpec, normalize: bool) -> Result<(), String> {
    match spec.kind.as_str() {
        "pcm8" => {
            let path = spec.args.first().ok_or("pcm8 needs file path")?;
            let mut bytes = fs::read(path).map_err(|e| e.to_string())?;
            if normalize {
                normalize_samples(&mut bytes);
            }
            builder
                .add_effect(
                    spec.id,
...
fn print_help() {
    println!(
        "eaf-bake — build EAFX v2 sound banks\n\n\
         ...
         --gen-c sfx_ids.h\n\n\
         Flags:\n  \
         --gen-rust PATH  Output Rust SFX ID constants\n  \
         --gen-c PATH     Output C header SFX ID constants\n  \
         --normalize      Peak normalize PCM8/ADPCM inputs\n\n\
